...
VulkanContext VulkanContextBuilder::build()
{
    ...
    if (m_enableValidation)
    {
        vk::DebugUtilsMessengerCreateInfoEXT debugMessengerCreateInfo(
            {}, m_messageSeverity, m_messageType, debugCallback);
        debugMessenger =
            instance.createDebugUtilsMessengerEXT(debugMessengerCreateInfo);
    }
    ...
    vk::DeviceCreateInfo      deviceCreateInfo(
        {}, queueCreateInfo, {}, m_deviceExtensions);
    ...
    return VulkanContext{std::move(context),
                         std::move(instance),
...
uint32_t VulkanContextBuilder::chooseGraphicsQueue(
    const vk::raii::PhysicalDevice& physicalDevice) const
{
    ...
    throw std::runtime_error{"There is no any graphics queue for device " +
