...
public:
    ...
    VulkanContextBuilder&
        setAppVersion(uint32_t major, uint32_t minor, uint32_t patch)
    {
    ...
    VulkanContextBuilder&
        setEngineVersion(uint32_t major, uint32_t minor, uint32_t patch)
    {
    ...
    VulkanContextBuilder&
        setApiVersion(uint32_t major, uint32_t minor, uint32_t patch)
    {
    ...
    VulkanContextBuilder&
        setValidationMessageSeverity(vk::DebugUtilsMessageSeverityFlagsEXT severity)
    {
    ...
    VulkanContextBuilder&
        setValidationMessageType(vk::DebugUtilsMessageTypeFlagsEXT type)
    {
        m_messageType = type;
        return *this;
    }
...
private:
    ...
    uint32_t
        chooseGraphicsQueue(const vk::raii::PhysicalDevice& physicalDevice) const;
};
